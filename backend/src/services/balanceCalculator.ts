...
interface BalanceTraceItem {
  type: 'paid' | 'owed' | 'settlement_paid' | 'settlement_received';
  expenseId?: string;
  settlementId?: string;
  description: string;
  date: Date;
...
export async function calculateBalances(groupId: string) {
  ...
  const membersMap = new Map<string, string>(); // userId -> userName
