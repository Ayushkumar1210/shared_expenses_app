...
export interface RawCsvRow {
  ...
  currency?: string;
  splitType?: string;
  splits?: string; // Format e.g., "Aisha:50, Rohan:50" or "Aisha, Rohan"
...
export function detectAnomalies(
    ...
    if (dateStr) {
      ...
      if (parts.length === 3) {
        ...
        if (third > 1000) {
          ...
          if (first > 12 && first <= 31) hasFirstFieldDay = true;
          if (second > 12 && second <= 31) hasSecondFieldDay = true;
        }
      }
    }
  });
  ...
  const parseDateString = (
    ...
    rowIdx: number
  ): { date: Date | null; isAmbiguous: boolean; error: string | null } => {
  ...
  const matchMember = (
  ...
  ): { member: GroupMember | null; isNormalized: boolean } => {
    if (!nameStr || nameStr.trim() === '') return { member: null, isNormalized: false };
    ...
    const partial = members.find(
      (m) =>
        cleaned.startsWith(m.name.toLowerCase()) ||
  ...
  rows.forEach((row, idx) => {
    ...
    const rawSplits: { rawName: string; value: number }[] = [];
    ...
    if (splitsStr === '') {
      ...
      const activeMembersOnDate = members.filter((m) => {
        ...
        if (expTime < joinTime) return false;
        if (m.leftAt && expTime > new Date(m.leftAt).getTime()) return false;
        return true;
      });
    ...
    const parsedShares: { user: GroupMember; value: number }[] = [];
    ...
    let shareAmountINR: number = 0;
    const finalShares: any[] = [];
    ...
    try {
      ...
      const calculated = calculateSplits(amount, splitType, splitInputs);
