interface InputShare {
  userId: string;
  value: number; // exact amount, percentage, or weight
}

interface CalculatedShare {
  userId: string;
  originalShare: number; // original input value
  shareAmount: number; // calculated share in original currency
}
...
export function calculateSplits(
  ...
  const calculated: CalculatedShare[] = [];
